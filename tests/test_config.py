...
def test_the_design_version_is_pinned_deliberately():
    ...
    **What it deliberately cannot catch, and this is worth knowing before relying on it:** a
    bump that never happened. Change the design document to v0.6 and touch neither this line
    nor ``defaults.toml``, and the suite stays green while every manifest claims v0.5. The
    ...
    """
    assert load_config().design_version == "v0.5"
